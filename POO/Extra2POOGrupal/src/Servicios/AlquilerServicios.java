...
public class AlquilerServicios {
    ...
    public Alquiler crearAlquiler(Pelicula[] listaPeli, PeliculaServicios ps) {

        System.out.println("El listado de las peliculas disponibles son:");
        ...
        do {
            ...
            peliAlquilada = leer.nextInt();

        } while ((peliAlquilada - 1) > listaPeli.length);

        System.out.println("Cuantos dias va a tener la pelicula?");
        double dia = leer.nextDouble();

        double precio = totalServicio(dia);

        String peliculaAlquilada = listaPeli[peliAlquilada - 1].getTitulo();
        listaPeli[peliAlquilada - 1].setDisponibilidad(false);

        return new Alquiler(peliculaAlquilada, precio, LocalDate.now(), LocalDate.now().plusDays((long) dia));

    }

    public double totalServicio(double dia) {
